...
import { X, Edit, AlertCircle, CheckCircle, Upload } from 'lucide-react';
...
const EditCourseModal: React.FC<EditCourseModalProps> = ({ course, onClose, onSuccess }) => {
  ...
  const [thumbnailPreview, setThumbnailPreview] = useState<string>(
    course.thumbnail ? pb.files.getUrl(course, course.thumbnail) : ''
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await pb.collection('courses').update(course.id, courseData);
      ...
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl border-2 border-gray-600 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-gray-800 to-gray-900 border-b-2 border-gray-600 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold text-amber-300">Edit Course</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {success && (
            <div className="bg-green-900/30 border-2 border-green-500 text-green-300 px-4 py-3 rounded-xl flex items-start">
              ...
              <div>
                <p className="font-semibold mb-1">Course Updated Successfully!</p>
